...
func (b *ball) update(mouseX, mouseY int32, dragging bool) {
	...
	for i := 0; i < numVerts; i++ {
		...
		if dragging && contains(b.draggedVertices, i) {
			v.x = float64(mouseX)
			v.y = float64(mouseY)
			v.vx = 0
			v.vy = 0
		...
		for nd := 1; nd < (numVerts / 8); nd++ {
			weight := springK / (float64(nd) * 2)
		...
		// Pull toward neighbors
		// next := &b.vertices[(i+1)%numVerts]
		// prev := &b.vertices[(i-1+numVerts)%numVerts]

		// neighborFX := neighborK * ((next.x+prev.x)/2 - v.x)
		// neighborFY := neighborK * ((next.y+prev.y)/2 - v.y)

		// next2 := &b.vertices[(i+2)%numVerts]
		// prev2 := &b.vertices[(i-2+numVerts)%numVerts]

		// neighbor2FX := neighborK2 * ((next2.x+prev2.x)/2 - v.x)
		// neighbor2FY := neighborK2 * ((next2.y+prev2.y)/2 - v.y)

		// v.vx += (forceX + neighborFX + neighbor2FX)
		// v.vy += (forceY + neighborFY + neighbor2FY)
...
func (b *ball) tryStartDrag(mouseX, mouseY int32) bool {
	const dragRadius = 100.0 // radius in pixels
	const dragRadiusSq = dragRadius * dragRadius

	b.draggedVertices = nil // reset dragged list

	for i, v := range b.vertices {
		dx := float64(mouseX) - v.x
		dy := float64(mouseY) - v.y
		distSq := dx*dx + dy*dy

		if distSq < dragRadiusSq {
			b.draggedVertices = append(b.draggedVertices, i)
		}
	}

	return len(b.draggedVertices) > 0
}
